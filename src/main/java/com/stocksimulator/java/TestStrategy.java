...
public class TestStrategy extends Strategy {
	...
	public TestStrategy(Market market, Parameters param) {
		super(market, param);
	...
	public void onStart() {
		...
		spread = (Integer) getParam("spread");
		hardLimit = (Integer) getParam("hardLimit");
		lastAskPrice = 0;
		lastBidPrice = 0;
		myLimit = 0;
		lastBuy = 0;
	...
	public void onQuotes() {
		StockInfo indcInfo = getSymbol(indc1);

		if (indcInfo instanceof Quote) {
			Quote q = (Quote) indcInfo;
			Position pos = getPosition(winc1);
			...
			if (qtd < hardLimit) {
				provideBuyLiquidity(winc1, 1, buyPrice);
			}
			if (qtd > -hardLimit) {
				provideSellLiquidity(winc1, 1, sellPrice);
			}
